function showResearch(searchTerm){
    $("#tab32").css("visibility", "visible");
    $("#researchNavLink").css("visibility", "visible");

    $("#tab21").removeClass("active");
    $("#tab22").removeClass("active");
    $("#tab32").addClass("active");

    $("#tabs-right").find(".active").removeClass("active");
    $("#researchNavLink").addClass("active");
    research(searchTerm)
}

function hideResearch(){
    $("#tab32").css("visibility", "hidden");
    $("#researchNavLink").css("visibility", "hidden");

    $("#tab21").addClass("active");
    $("#tab32").removeClass("active");

    $("#researchNavLink").removeClass("active");
    $("#second-tab").find(".nav-link").first().addClass("active");

    $('#resultAreaWiki').empty();
}
...
function research(searchTerm){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $('#resultAreaWiki').html(`${newTitle}<div>${data.query.pages[dataNum].extract}</div>`);
        },
        complete: function(){
            /*
